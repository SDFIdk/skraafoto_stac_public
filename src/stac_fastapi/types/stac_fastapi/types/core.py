...
class BaseCoreClient(LandingPageMixin, abc.ABC):
    ...
    def get_extension(self, extension: str) -> ApiExtension:
        extensions = [ext for ext in self.extensions if type(ext).__name__ == extension]
        if extensions:
            return extensions[0]
        else:
    ...
    @abc.abstractmethod
    def item_collection(
        self, collection_id: str, limit: int = 10, pt: str = None, **kwargs
    ) -> stac_types.ItemCollection:
        ...
        landing_page["links"].append(
            {
                ...
                "type": "application/vnd.oai.openapi+json;version=3.0",
                "title": "OpenAPI service description",
                "href": urljoin(base_url, "/ogc/openapi.json"),
            }
        )
...
class AsyncBaseFiltersClient(abc.ABC):
    ...
    @abc.abstractmethod
    async def validate_collection(self, value):
        ...
